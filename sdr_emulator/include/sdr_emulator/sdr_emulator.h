...
private:
  ...
  //=> эмулятор работает только пакетно, при вызове stop() ждем пока заполнится
  // и обработается 1 пакет
  void receivePackage(uint8_t *package_begin, size_t valid_length);
  void receiveCounterPackage(uint8_t *package_begin, size_t valid_legth);
